...
void set_interrupt_gate(
    ...
    uint32_t address = (uint32_t)handler_address;
    idt_int_gate->offset_high = (uint16_t)(address >> 16 & 0xFFFF);
    idt_int_gate->offset_low = (uint16_t)(address & 0xFFFF);
    ...
    idt_int_gate->privilige   = privilege;
